...
class WithdrawWire extends AbstractOperation
{
    ...
    protected function feeFromClientToWallet()
    {
        ...
        $this->providerFeeAmount = $this->_commissionService->calculateCommissionAmount($toCommission, $this->operationAmount);
        $transactionService =  $this->_transactionService;

        $this->_systemFeeFromClientToWalletProvider = $transactionService->createTransactions(
            TransactionType::SYSTEM_FEE, getCorrectAmount($this->providerFeeAmount, $this->_operation->from_currency), $this->_operation->fromAccount, $providerAccount->childAccount, $this->date,
        ...
        if ($toCommission->blockchain_fee) {
            $this->_blockchainFeeFromClientToWalletProvider = $transactionService->createTransactions(
                TransactionType::BLOCKCHAIN_FEE, getCorrectAmount($toCommission->blockchain_fee, $this->_operation->from_currency), $this->_operation->fromAccount, $providerAccount->childAccount, $this->date,
                TransactionStatuses::SUCCESSFUL, null, $this->_operation, null, $toCommission->id,
                'Client', 'Wallet provider fee', $this->_operation->step
            );
        }
    ...
    protected function sendFromClientToLiquidity()
    {
        ...
        $this->_transaction = $this->makeCryptoTransaction($fromAccount, $liquidityAccount, getCorrectAmount($this->leftAmount, $this->_operation->from_currency), 'Client wallet', 'Liquidity crypto');
