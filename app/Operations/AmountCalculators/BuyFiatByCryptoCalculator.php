...
class BuyFiatByCryptoCalculator extends AbstractOperationCalculator
{
    ...
    {
        $paymentTransaction = $this->_operation->getPaymentTransaction();

        $fromAccount = $paymentTransaction->toAccount ?? null;
        if (!$fromAccount) {
            return 0;
        }
    ...
    {
        $transactionSystemAmount = $this->getWalletProviderFeeAmount();
    ...
    public function getProviderPaymentProviderFeePercentCommission()
    {
        ...
        return  $paymentTransaction->fromCommission->percent_commission ?? null;
    }
