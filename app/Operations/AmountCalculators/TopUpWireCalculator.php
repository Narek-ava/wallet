...
class TopUpWireCalculator extends AbstractOperationCalculator
{
    ...
    public function getCratosFeeAmountFiat(): float
    {
        ...
        $fromWireProviderToSystemFeeTransaction =
            $this->_operation->getTransactionByAccount(TransactionType::SYSTEM_FEE,
                TransactionStatuses::SUCCESSFUL, $providerAccount->id, null);

        if (!$fromWireProviderToSystemFeeTransaction) {
            return 0;
        }

        $fromSystemToWireProviderFeeTransaction =
            $this->_operation->getAllTransactionsByProviderTypesQuery(false, null,
                Providers::PROVIDER_PAYMENT)->sum('trans_amount');


        $fromSystemToLiquidityProviderFeeTransaction =
            $this->_operation->getAllTransactionsByProviderTypesQuery(false, null,
                Providers::PROVIDER_LIQUIDITY)->sum('trans_amount');

        $amount = $fromWireProviderToSystemFeeTransaction->trans_amount;
        $amount -= $fromSystemToWireProviderFeeTransaction;
        $amount -= $fromSystemToLiquidityProviderFeeTransaction;
