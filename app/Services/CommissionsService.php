...
class CommissionsService
{
    ...
    public function getLiquidityProviderOutgoingCommission(string $currency, ?string $projectId = null): ?Commission
    {
        $liqProviderAccount = Account::getProviderAccount($currency, Providers::PROVIDER_LIQUIDITY, null, null, $projectId);
        return $liqProviderAccount->getAccountCommission(true);
    }

    public function getLiqProviderCommissionFee(string $currency, float $amount,  ?string $projectId)
    {
       $liqProviderCommission = $this->getLiquidityProviderOutgoingCommission($currency, $projectId);

       if (!$liqProviderCommission) {
           return 0;
       }

       return $this->calculateCommissionAmount($liqProviderCommission, $amount);
