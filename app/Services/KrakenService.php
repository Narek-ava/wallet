...
class KrakenService implements ExchangeInterface
{
    ...
    protected $secret;
    protected $url;
    protected $liquidityProvider;
    protected $version;
    protected $curl;

    function __construct(PaymentProvider $liquidityProvider, $url = 'https://api.kraken.com', $version = '0', $sslverify = true)
    {
        $this->liquidityProvider = $liquidityProvider;
        $this->key = $this->getConfigValue('api_key');
        $this->secret = $this->getConfigValue('api_secret');
        $this->url = $url;
    ...
    {
        $configKey = 'liquidityproviders.' . $this->liquidityProvider->api . '.' . $this->liquidityProvider->api_account . '.' . $key;
    ...
    public function QueryPrivate($method, array $request = array())
    {
        ...
        if (empty($result['result'])) {
            ...
            logger()->error('KrakenRequestError', $request);
    ...
    protected function validateCurrencies(string $from, string $to)
    {
        if (
            ...
            (Currency::getName($from) && Currency::getName($to))
        ) {
    ...
    public function getRateCryptoFiat(string $from, string $to, $amount): string
    {
        $from = $this->getGetFormattedCurrency($from);
        $to = $this->getGetFormattedCurrency($to);
    ...
    public function getGetFormattedCurrency(string $currency)
    {
        if ($currency == Currency::CURRENCY_MCDAI) {
            $currency = self::COIN_DAI;
        }
    ...
    {
        $this->fixFromAndToCurrencies($coin, $fiat, $type);
    ...
    private function getCoin($coin)
    {
        ...
        if (strtoupper($coin) === Currency::CURRENCY_MCDAI) {
            return self::COIN_DAI;
        }
        return $coin;
    }
    ...
    {
        $fromCurrency = Currency::KRAKEN_ASSETS[$fromCurrency] ?? $fromCurrency;
        $toCurrency = Currency::KRAKEN_ASSETS[$toCurrency] ?? $toCurrency;
        ...
        if (config('app.env') == 'local') {
            ...
            if (in_array($toCurrency, Currency::FIAT_CURRENCY_NAMES)) {
                $amount *= $rateAmount;
            }
    ...
    ): string
    {
        ...
        $res = $this->QueryPrivate('TradeVolume', array('pair' => $coin.$rate, 'since' => $since));
        try {
            $precent = array_values($res['result']['fees'])[0];
