...
class TicketMessageService
{
    ...
    public function storeMessage($data, $creator)
    {
        $ticketMessage = TicketMessage::create([
            ...
            'ticket_id' => $data['ticket-id'] ??  $data['ticket_id'],
    ...
    public function viewTicketMessages($id, $type)
    {
        $ticketMessages = TicketMessage::where(['ticket_id' => $id, 'viewed' => false])
            ->whereHas('ticket', function ($q){
                return $q->where('to_client', auth()->user()->id);
            })
            ->where('massageable_type', '!=', $type)->get();

        if ($ticketMessages->isNotEmpty()) {
            TicketMessage::query()
                ->whereIn('id', $ticketMessages->pluck('id')->toArray())
                ->update(['viewed' => true]);
