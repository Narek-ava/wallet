...
class BitGOAPIService
{
    ...
    protected function getCoinSpecifications(string $coin, bool $withQuery = true): array
    {
        $coin = strtoupper($coin);
        $specifications['query'] = [];
        if (in_array($coin, Currency::MAIN_CRYPTOCURRENCIES)) {
            if (isset(Currency::TOKENS_WITH_SUBTOKENS[$coin])) {    //parent token
                $specifications['onToken'] = strtolower($coin);
                $specifications['query'] = $withQuery ? [
                    'allTokens' => true
                ] : [];
            }
            $specifications['coin'] = strtolower($coin);
        } else {
            foreach (Currency::TOKENS_WITH_SUBTOKENS as $cryptocurrency => $subtokens) {
                if (in_array(strtoupper($coin), $subtokens)) {
                    $specifications['coin'] = strtolower($cryptocurrency);
                    $specifications['onToken'] = strtolower($coin);
                    $specifications['query'] = $withQuery ? [
                        'allTokens' => true
                    ] : [];
    ...
    protected function getResult(string $url, string $httpMethod = 'GET', array $postFields = [], array $queryFields = [])
    {
    ...
    protected function getResultExpress(string $url, string $httpMethod = 'GET', array $postFields = [], array $queryFields = [])
    {
        $data = $this->getRequestHeaderAuth() ?? [];
        if ($postFields) {
            $data['json'] = $postFields;
...
//        $this->unlockSession();
        ...
        try {
            $decryptedPassphrase = Crypt::decrypt($fromWallet->passphrase);
        } catch (\Exception $exception) {
            logger()->error('PassphraseDecryptionError', ['message' => $exception->getMessage()]);
            throw new OperationException($exception->getMessage(), $exception->getCode(), $exception);
        }
        $data = [
            'address' => $toWallet->address,
            'amount' => bcmul((string)$amount,  (string)Currency::BASE_CURRENCY[$toWallet->coin]),
            'walletPassphrase' => $decryptedPassphrase, //TODO check
    ...
    public function generateWallet(string $coin, array $data = [])
    {
        $coin = strtolower($coin);
        if (config('app.env') == 'local') {
            return json_encode(['id' => Str::random(), 'coin' => $coin, 'label' => $data['label'], 'receiveAddress' => ['address' => Str::random(), "wallet" => "59cd72485007a239fb00282ed480da1f"]], true);
        }
        return $this->getResultExpress("api/v2/{$coin}/wallet/generate", "POST", $data);
    }

    public function generateWalletAddress(string $coin, string $walletId, ?string $label = null)
    {
        $coinSpecifications = $this->getCoinSpecifications($coin);
        $coin = $coinSpecifications['coin'];

        $body = ['label' => $label];
        if (isset(Currency::TOKENS_WITH_SUBTOKENS[strtoupper($coin)])) {
            $body['onToken'] = $coinSpecifications['onToken'];
        }

        if (config('app.env') == 'local') {
            return json_encode(['label' => $label, 'address' => Str::random(), 'coin' => $coin]);
        }

        return $this->getResultExpress("api/v2/{$coin}/wallet/{$walletId}/address", 'POST', $body, $coinSpecifications['query']);
    }
...
//    }
    ...
    public function listTransfers(string $coin, string $walletId, ?string $address = null)
    {
        $coinSpecifications = $this->getCoinSpecifications($coin);
        $query = $coinSpecifications['query'];
        $query['address'] = $address;
        $coin = strtolower($coin);

        return json_decode($this->getResult("api/v2/{$coin}/wallet/{$walletId}/transfer", 'GET', [], $query), true);
    }
    ...
    public function addWalletWebhook(string $coin, string $walletId, string $type, string $url)
    {
        ...
        return $this->getResult("api/v2/{$coin}/wallet/{$walletId}/webhooks",'POST', $data, $coinSpecifications['query']);
    }
    ...
    public function listWalletWebhooks(string $coin, string $walletId)
    {
        $coinSpecifications = $this->getCoinSpecifications($coin);
        $coin = $coinSpecifications['coin'];
        return json_decode($this->getResult("api/v2/{$coin}/wallet/{$walletId}/webhooks", 'GET', [], $coinSpecifications['query']), true);
    }


    public function removeWalletWebhook(string $coin, string $walletId, array $data = [])
    {
        $coinSpecifications = $this->getCoinSpecifications($coin);
        $coin = $coinSpecifications['coin'];
        return $this->getResult("api/v2/{$coin}/wallet/{$walletId}/webhooks","DELETE", $data);
    }
    ...
    public function getCryptoTransferData(array $transfer, ?string $monitorAddress = null): CryptoTransferData
    {

        if (!empty($transfer['entries']) && count($transfer['entries']) == 2) {
            foreach ($transfer['entries'] as $entry) {
                if ($entry['value'] < 0) {
                    ...
                    if ($address === $monitorAddress) {
                        $transfer['type'] = BitGOAPIService::IS_RECEIVE_TRANSFER;
                        $transfer['value'] = $entry['value'];
                    }
                }
            }
        } elseif (!empty($transfer['outputs'])) {
            foreach ($transfer['outputs'] as $entry) {
                if (isset($entry['chain']) && intval($entry['chain']) > 0) {
                    continue;
                }
                if (isset($entry['address']) && $entry['value'] > 0) {
                    $address = $entry['address'];
                    if ($address === $monitorAddress) {
                        $transfer['type'] = BitGOAPIService::IS_RECEIVE_TRANSFER;
                        $transfer['value'] = $entry['value'];
                ...
                if (isset($entry['chain']) && $entry['chain'] == 0) {
                    break;
                }
            }
        }
        $dto = new CryptoTransferData([
            'is_received' => $transfer['type'] == BitGOAPIService::IS_RECEIVE_TRANSFER,
            'is_approved' => $transfer['state'] == BitGOAPIService::TRANSFER_IS_APPROVED,
            'from_address' => $fromAddress ?? $transfer['inputs'][0]['address'] ?? null,
            'to_address' => $address ?? null,
            'tx_id' => $transfer['txid'] ?? null,
            'value' => $transfer['value'] ?? null,
            'is_init' =>  isset($transfer['entries'][0]['type']) && strpos($transfer['entries'][0]['type'], 'Initialization') !== false
        ]);

        if ($dto->is_init) {
            $dto->is_received = false;
        }

        if (!$dto->is_init && (!$dto->from_address || !$dto->to_address)) {
            logger()->error('MissingAddressCryptoTransfer', $transfer);
        }
