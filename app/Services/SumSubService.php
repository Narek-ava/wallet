...
class SumSubService
{
    ...
    const ACCOUNT_TYPES_IN_SUMSUB = [
        CProfile::TYPE_INDIVIDUAL => 'individual',
        CProfile::TYPE_CORPORATE => 'company',
    ];
    ...
    {
        $url = "/resources/accessTokens?userId=" . $externalUserId . '&levelName=' . urlencode($levelName)  . '&ttlInSecs=3600';
    ...
    public function getApplicantName($applicantId): string
    {
        $applicantData = $this->getApplicantInfo($applicantId);
        if ($applicantData['type'] === self::ACCOUNT_TYPES_IN_SUMSUB[CProfile::TYPE_CORPORATE]) {
            return $applicantData['info']['companyInfo']['companyName'] ?? '';
        }
        return ($applicantData['info']['firstNameEn'] ?? ($applicantData['fixedInfo']['firstName'] ?? '')) . ' ' . ($applicantData['info']['lastNameEn'] ?? ($applicantData['fixedInfo']['lastName'] ?? ''));
    }
     ...
     * @return string|null
     */
    public function getToken(string $profileId, string $levelName): ?string
    {
        $arr = json_decode($this->getAccessToken($profileId, $levelName), true);
    ...
    public function getNextLevelName(int $accountType, int $complianceLevel, ?ComplianceRequest $retryComplianceRequest = null, ?ComplianceRequest $lastRequestIfDeclined = null): ?string
    {
        ...
        if ($complianceNextLevel == ComplianceLevel::VERIFICATION_LEVEL_0) {
            $complianceNextLevel = ComplianceLevel::VERIFICATION_LEVEL_1;
        }

        if ($complianceNextLevel > ComplianceLevel::VERIFICATION_LEVEL_3) {
            $complianceNextLevel = ComplianceLevel::VERIFICATION_LEVEL_3;
        }
    ...
    public function getApplicantInfoByExternalUserId($externalUserId)
    {
    ...
    public function getShareToken($applicantId, $fromClient)
    {
        $url = "/resources/accessTokens/-/shareToken?applicantId={$applicantId}&forClientId={$fromClient}";
        $request = new Request('POST', $this->apiUrl . $url);

        return json_decode($this->sendHttpRequest($request, $url)->getBody()->getContents(), true);
    }

    public function importApplicant($shareToken, $applicantId, $trustReview = 'false')
    {
        $url = "/resources/applicants/-/import?shareToken={$shareToken}&trustReview={$trustReview}&userId={$applicantId}";
        $request = new Request('POST', $this->apiUrl . $url);
