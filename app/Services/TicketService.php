...
class TicketService
{
    ...
    public function closeTicket($id)
    {
        ...
        if (get_class(auth()->user()) == CUser::class) {
            $ticket = $ticket->where('to_client', auth()->user()->id);
        }
        $ticket = $ticket->first();
    ...
    public function ticketsPaginate($request, $status)
    {
        ...
        if ($projectId){
           $queryTicket->whereHas('user', function ($query) use ($projectId) {
     ...
     * @return mixed
     */
    public function getTicketsByClient($params)
    {

        if(!empty($params['ticket_status'])) {
            if($params['ticket_status'] == TicketStatuses::STATUS_CLOSE) {
                $queryTickets = auth()->user()->closedTickets();
            } else{
                $queryTickets = auth()->user()->openTickets();
            }
        } else {
            $queryTickets = auth()->user()->tickets();
        }

        if (!empty($params['search'])) {
            $queryTickets->where('subject', 'like', '%' . $params['search'] . '%');
        }
        return $queryTickets->paginate(config('cratos.pagination.tickets'));
    }
