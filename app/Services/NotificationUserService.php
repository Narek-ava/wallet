...
class NotificationUserService extends CoreNotification
{
    ...
    public function createNotificationUserRecord(array $userIds, string $notificationId, string $userType)
    {
        foreach ($userIds as $userId) {
            $this->create($userId, $notificationId, $userType);
        }
    }
    ...
    public function getNotificationUsersViewedDataWithPaginate($filters)
    {
        $query = $this->getModel()->where('status', NotificationStatuses::VIEWED)->whereHas('notification', function($q) use ($filters) {
            ...
            if (!empty($recipients)) {
                $q->whereIn('recepient', $recipients);
            }
    ...
    public function getUserIdsArray($recepient,$projectId = null)
    {
        /* @var CUserService $cUserService */
        $cUserService = resolve(CUserService::class);
        switch ($recepient) {
            case NotificationRecipients::ALL_CLIENTS:
                return $cUserService->getUserIdsArray($projectId);
            case NotificationRecipients::ALL_USERS:
                $bUsersService = resolve(BUsersService::class);
                return $bUsersService->getUserIdsArray($projectId);
            case NotificationRecipients::ALL_CORPORATE:
                return $cUserService->getUsersByType(CProfile::TYPE_CORPORATE, $projectId);
            case NotificationRecipients::ALL_INDIVIDUAL:
                return $cUserService->getUsersByType(CProfile::TYPE_INDIVIDUAL, $projectId);
        }
        return [];
    }


    public function getUserEmailsArray($recipient, $projectId = null)
    {
        switch ($recipient) {
            case NotificationRecipients::ALL_CLIENTS:
                return (new CUserService)->getUserEmailsArray($projectId);
            case NotificationRecipients::ALL_USERS:
                return (new BUsersService)->getUserEmailsArray($projectId);
            case NotificationRecipients::ALL_CORPORATE:
                return (new CUserService)->getUsersByType(CProfile::TYPE_CORPORATE, $projectId);
            case NotificationRecipients::ALL_INDIVIDUAL:
                return (new CUserService)->getUsersByType(CProfile::TYPE_INDIVIDUAL, $projectId);
        }
        return [];
    }
