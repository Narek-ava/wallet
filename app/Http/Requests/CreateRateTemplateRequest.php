...
use App\Enums\{Commissions, CommissionType, ComplianceLevel, Currency};
...
class CreateRateTemplateRequest extends FormRequest
{
    ...
    protected function getValidationStringForCurrency(string $currentCurrency): string
    {
        ...
        $currencies = in_array($currentCurrency, Currency::FIAT_CURRENCY_NAMES) ? Currency::getAllCurrencies() : Currency::getList();
    ...
    public function rules()
    {
        ...
        foreach (ComplianceLevel::NAMES as $level => $name) {
            $keyTransactionAmount = 'transaction_amount_max.' . $level;
            $ruleTransactionAmount = 'required_without_all:monthly_amount_max.' . $level;
            $additionalRules[$keyTransactionAmount] = ['bail', $ruleTransactionAmount, 'numeric', 'nullable'];

            $keyMonthlyAmount = 'monthly_amount_max.' . $level;
            $ruleMonthlyAmount = 'required_without_all:transaction_amount_max.' . $level;
            $additionalRules[$keyMonthlyAmount] = ['bail', $ruleMonthlyAmount, 'numeric', 'nullable'];
        }
    ...
    public function messages()
    {
        ...
        foreach (ComplianceLevel::getList() as $level => $name) {
            $messages["transaction_amount_max.{$level}.required_without_all"] = t('limit_field_required_without_all', ['level' => $level]);
            $messages["transaction_amount_max.{$level}.numeric"] = t('provider_field_numeric');
            $messages["monthly_amount_max.{$level}.required_without_all"] = t('limit_field_required_without_all', ['level' => $level]);
            $messages["monthly_amount_max.{$level}.numeric"] = t('provider_field_numeric');
