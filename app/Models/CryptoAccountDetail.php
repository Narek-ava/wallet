...
use App\Services\{AccountService, BitGOAPIService, WalletService};
use Carbon\Carbon;
...
class CryptoAccountDetail extends BaseModel
{
    ...
    public static function getAllowedCoinsForAccount()
    {
        ...
        $projectSystemWallets = $project->clientSystemWallets()->whereNotNull('wallet_id')->pluck('currency')->toArray();

        /* @var WalletService $walletService */
        $walletService = resolve(WalletService::class);

        foreach ($allAllowedCoins as $coin) {
            if (in_array($coin, $accountCoins) || !in_array($coin, $projectSystemWallets)) {
                unset($allAllowedCoins[$walletService->getConfigs($project->id, 'coin_prefix') . strtolower($coin)]);
            }
        }
    ...
    {
        return !is_null($this->risk_score) && $this->risk_score <= floatval(config(AccountService::CONFIG_RISK_SCORE));
    }
    ...
    public function getWalletBalance(): ?string
    {
        ...
        $key = 'balance_'.$this->wallet_id . '_' . $this->coin;
        ...
        if (is_null($balance)) {
            ...
            try {
                ...
                $availableBalance = $walletData['spendableBalance'] ?? $walletData['spendableBalanceString'] ?? 0;
