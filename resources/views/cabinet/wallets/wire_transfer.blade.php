    ...
    <div class="row">
        <div class="col-md-12">
            <form id="regForm" method="post"
                  action="{{ route('cabinet.wallets.wire.transfer', $cryptoAccountDetail->id ) }}">
                @csrf
                <div class="tab">
                ...
                <div class="tab">
                    ...
                    <div class="row mt-5">
                        ...
                        <div class="col-md-6 col-lg-3 mb-4">
                            ...
                            <input type="number" min="0" class="amount" name="amount" placeholder="Amount"
                        ...
                        <div class="col-md-6 col-lg-3 mb-4">
                            ...
                            <input type="text" min="0" class="w-100 p-1" readonly id="exchange_to" name="exchange_to" value="{{ $cryptoAccountDetail->coin }}"
        ...
        <div class="row">
            <div class="col-md-12 mt-4 pt-2">
                <label class="component ml-0 p-0 bank-details-label">
                    <input type="radio" name="bank_detail" class="provider_name d-none"
            ...
            if (n == 2) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        "amount": $('.amount').val(),
                        "accountType": $("input[type='radio'][name='wire_type']:checked").val(),
                    ...
                    success: function (response) {
                        ...
                        if (response.accountExist == false || (response.isAmountValid != null && response.isAmountValid == false)) {
                            nextPrev(-1);
                            if (response.accountExist == false) {
                                $('.no-providers-text').removeClass('d-none').addClass('d-block');
                            }else if (response.isAmountValid == false) {
                                $('.invalid-amount-text').removeClass('d-none').addClass('d-block');
                                $('.invalid-amount-text').text(response.invalidAmountMessage);
                            }
                        } else {
                            for (let i = 0; i < response.providers.length; i++) {
                                ...
                                if (provider.accounts.length != 0) {
                                    for (let j = 0; j < provider.accounts.length; j++) {
                                        ...
                                        $('.provider_name').val(account.id);
                                        $('.provider_name_text').html(`${provider.name} - ${account.name}`);
                                        $('.account_beneficiary').html(account.wire.account_beneficiary ?? '-');
                                        $('.beneficiary_address').html(account.wire.beneficiary_address ?? '-');
                                        if(!response.isTypeSwift) {
                                            $('.iban_eur').html(account.wire.iban ?? '-');
                                        ...
                                        $('.swift_bic').html(account.wire.swift ?? '-');
                                        $('.bank_name').html(account.wire.bank_name ?? '-');
                                        $('.bank_address').html(account.wire.bank_address ?? '-');
                                        ...
                                        if(account.account_type == accountType) {
                                            ...
                                            $('.correspondent_bank').html(account.wire.correspondent_bank ?? '-');
                                            $('.correspondent_bank_swift').html(account.wire.correspondent_bank_swift ?? '-');
                                            $('.intermediary_bank').html(account.wire.intermediary_bank ?? '-');
                                            $('.intermediary_bank_swift').html(account.wire.intermediary_bank_swift ?? '-');
                                        }
                    ...
                    success: function (response) {
                        ...
                        if (response.message == 'failed') {
                            ...
                            if (response.blockChainFee) {
                                blockchainFee += (response.blockChainFee * blockChainCount)
                            }
                            blockchainFee = blockchainFee.toFixed(8);
                            $('.blockchain-fee').text(blockchainFee + ' ' + (response.blockChainFeeCurrency ?? ''))
                            $('.topup-fee').text((response.commissions.percent_commission ?? '-') + '%' + (response.commissions.min_commission ? ' ( Min. ' + response.commissions.min_commission + ' ' + response.commissions.currency+ ')' : '') );
                            $('.transactionLimit').text(response.transactionLimit ?? '-');
                        }
                    }
                });
            } else if (n == 3) {
                if(!$("input[type='radio'][name='bank_detail']:checked").val()){
                ...
                $.ajax({
                    ...
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "provider_account_id": $("input[type='radio'][name='bank_detail']:checked").val(),
                    },
                    success: function (response) {
