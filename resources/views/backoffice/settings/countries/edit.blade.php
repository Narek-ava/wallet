...
@section('content')
    ...
    <div class="row mt-5">
        <div class="col-md-6">
            <form action="{{ route('countries.update', ['country' => $country]) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <h3> {{ t('ui_country_edit') }} </h3>
                ...
                <div class="form-group mt-5">
                    <div class="form-label-group">
                        ...
                        <input id="inputCode" name="code" type="text" value="{{ $country->code }}"
                 ...
                 <div class="form-group mt-5">
                    <div class="form-label-group">
                        ...
                        <input id="inputCodeISO3" name="code_ISO3" type="text" value="{{ $country->code_ISO3 }}"
                               class="form-control{{ $errors->has('key') ? ' is-invalid' : '' }}" required>
                    </div>
                    <br>
                    @error('code_ISO3')
                    ...
                    <div class="form-label-group">
                        ...
                        <select name="isBanned" id="inputBanned">
                            @foreach(\App\Models\Country::BANNED_NAMES as $key => $name)
                                <option value="{{ $key }}"
                                        @if($country->is_banned === $key) selected @endif>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <br>
                    ...
                    <div class="form-label-group">
                        ...
                        <select name="isAlphanumericSender" id="inputAlphanumericSender">
                            @foreach(\App\Models\Country::ALPHANUMERIC_SENDER_NAMES as $key => $name)
                                <option value="{{ $key }}"
                                        @if($country->is_alphanumeric_sender === $key) selected @endif>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <br>
                    ...
                    <div class="form-label-group">
                        ...
                        <div class="d-flex justify-center">
                            <input id="inputPhoneCode" name="phoneCode" type="text"
                                   value="{{ implode(', ', $country->phone_code ?? []) }}"
                                   class="form-control{{ $errors->has('key') ? ' is-invalid' : '' }}" required>
                        </div>
                    </div>
                    <br>
                ...
                <div class="form-group mt-5">
                    <div class="form-label-group">
                        ...
                        <div class="d-flex justify-center">
                            ...
                            <img src="/cratos.theme/images/flag/{{ $country->flag }}" id="countryFlagUploadImage">
