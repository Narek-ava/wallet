        ...
        <div class="modal-content common-shadow-theme">
            ...
            <div class="modal-body">
                ...
                <form autocomplete="off" class="form " role="form" method="post"
                      action="{{route('backoffice.profile.store')}}">
                    {{ csrf_field() }}
                    <div class="row mb-5">
                        <div class="form-group col-md-4">
                            ...
                            <div class="d-flex">
                                <select name="manager_id" class="mr-3" style="width:280px;">
                                    ...
                                    <option  {{ (old('manager_id') == $id ? "selected":"") }} value="{{$id}}">{{$email}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="d-flex">
                                <select name="compliance_officer_id" class="mr-3" style="width:280px;">
                                    ...
                                    @foreach(\App\Models\Backoffice\BUser::complianceManagersList() as $id => $email)
                                        <option {{ (old('compliance_officer_id') == $id ? "selected":"") }} value="{{$id}}">{{$email}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <input class="form-check-input" type="hidden" name="account_type" id="individual" value="{{$type}}">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input autocomplete="off" class="form-control" type="text" name="last_name" required value="{{  old('last_name')}}">
                            @error('last_name')
                            <div class="error text-danger">{{ $message }}</div>
                            @enderror
                        </div>
