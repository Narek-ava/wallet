...
@section('content')
    ...
    <h4><a href="{{ route('backoffice.profile', $cProfile->id) }}" class="text-dark">{{t('ui_cprofile_profile_id')}} {{ $cProfile->profile_id }}</a> -
        <a href="{{ route('backoffice.profile.wallet', in_array($operation->operation_type , [ \App\Enums\OperationOperationType::TYPE_TOP_UP_CRYPTO, \App\Enums\OperationOperationType::TYPE_CRYPTO_TO_CRYPTO_PF])  ? $operation->toAccount->cryptoAccountDetail->id : $operation->fromAccount->cryptoAccountDetail->id ?? '') }}" class="text-dark">{{ strtoupper($operation->to_currency) }} {{t('title_wallet_page')}}</a>
    ...
    <div class="row mt-5">
        <div class="col-md-8">
            <div class="row common-shadow-theme pt-3 pb-3 ml-1">
                <div class="col-md-4 text-left px-3 ml-3">
                    ...
                    <p class="d-block">{{ t('crypto_payment_form')}}</p>
                    ...
                    <div class="col-md-4 text-left px-3 ml-3">
                        ...
                        <h6 class="d-block">{{ t('top_up_fee') }}</h6>
                        <p class="d-block">{{ ($operation->paymentFormAttempt->incoming_fee ?? '-') . '%' }}</p>
                        @if ($link)
                            <h6 class="d-block">{{ t('transaction_details') }}</h6>
                            <a href="{{ $link }}" class="text-nowrap"
                               target="_blank">{{ t('view') }}</a>
                        @endif
                    </div>
                <div class="col-md-3 text-left px-3 ml-3">
                    ...
                    <p class="d-block">{{ eur_format($availableMonthlyAmount) }}</p>
                        ...
                        <p class="d-block">{{ !$pendingCryptoTransaction ? ($operation->getLastTransactionByType(\App\Enums\TransactionType::CRYPTO_TRX)->trans_amount ?? 0) : 0 }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="row">
                <div class="col-md-12 p-3">
                    ...
                    @if($currentAdmin->isAllowed([\App\Enums\BUserPermissions::CHANGE_OPERATION_STATUS]))
    ...
    @if($fromWallet)
        <div class="row">
            ...
            <div class="col-md-6 mt-5 ml-3 d-flex">
                <div class="row common-shadow-theme pt-3 w-100">
                    <div class="col-md-12">
                        <div class="row float-right">
                        <span
                            class="font-weight-bold {{ $fromWallet->isAllowedRisk() ? 'text-success' : 'text-danger'}} mr-3">{{ $fromWallet->risk_score * 100 ?? '-'}} %</span>
                        </div>
                        <div class="row">
                            <div class="col-md-12 text-left">
                                <h6 class="d-block">KYT</h6>
                                <p class="d-block {{ $fromWallet->isAllowedRisk() ? 'text-success' : 'text-danger'}}">
                                    @if (!$fromWallet->isAllowedRisk())
                                ...
                                <p class="d-block"
                                   style="font-size: 14px;word-break: break-all;">{{ $fromWallet->address ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if((!$isCryptoToCryptoPF && $payerDetails) || ($isCryptoToCryptoPF && $operation->paymentFormAttempt))
                <div class="mt-5 d-flex">
                    <div class="col-md-12 d-flex">
                        <div class="row common-shadow-theme pt-4 pb-3 w-100">
                            <div class="col-md-12">
                                <h6 class="d-block">{{ t('payer_name') }}</h6>
                                <p class="d-block">{{ $isCryptoToCryptoPF ? $operation->paymentFormAttempt->getPayerFullName() : $payerDetails->getFullName() }}</p>
                                <h6 class="d-block">{{ t('payer_phone_number_backoffice') }}</h6>
                                <p class="d-block fs14">{{ $isCryptoToCryptoPF ? $operation->paymentFormAttempt->phone : $payerDetails->cUser->phone }}</p>
                                <h6 class="d-block">{{ t('email') }}</h6>
                                <p class="d-block fs14">{{ $isCryptoToCryptoPF ? $operation->paymentFormAttempt->email :$payerDetails->cUser->email }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="row mt-5">
            @if(!$operation->isLimitsVerified() ||
                ...
                $operation->status == \App\Enums\OperationStatuses::PENDING &&
                 ...
                 && ($currentAdmin->isAllowed([\App\Enums\BUserPermissions::CHANGE_OPERATION_STATUS]))
            )
                <div class="col-md-4">
                    <div class="row common-shadow-theme pt-3 pb-4">
                        <div class="col-md-12 text-left">
                            ...
                            <form action="{{ route('backoffice.approve.operation.status', $operation->id) }}"
                                  method="POST">
                                @csrf
                                <button class="btn btn-lg btn-primary themeBtn register-buttons mt-2 p-2 round-border"
                                        type="submit">
                            ...
                            <form action="{{ route('backoffice.withdraw.change.status', $operation->id) }}"
                                  method="POST">
                                @csrf
    ...
    <div class="row mt-5">
        ...
        @if(($currentAdmin->isAllowed([\App\Enums\BUserPermissions::ADD_TRANSACTION])))
            ...
            @include('backoffice.withdraw-crypto-transactions._add-transaction', ['address' => $fromWallet->address ?? ''])
        @endif
    </div>
