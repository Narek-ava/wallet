    ...
    <script>
        ...
        function getToAccountsByType($id) {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    // toAccountId.append('<option hidden value="">' + 'Select...' + '</option>');
                    toAccountId.append('<option hidden value="">' + 'Select...' + '</option>');

                    $('.to-account-option').val('');
                    for (let i = 0; i < response.toAccounts.length; i++) {
                        toAccountId.append('<option value="' + response.toAccounts[i].id + '" class="to-account-option">' + response.toAccounts[i].name + '</option>');
                    }
                }
            })
        }
        ...
        $('#to_account').on('change', function () {
            $.ajax({
                ...
                success: (response) => {
                   $('#to_address').val(response.address)
                }
            })
        })
