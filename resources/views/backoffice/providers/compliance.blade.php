...
@section('content')
    ...
    <div class="col-md-12">
        <h2 style="display: inline;margin-right: 25px;">{{ t('title_compliance_provider_page') }}</h2>
        ...
        @endif
        <p>
            <input type="checkbox" id="complianceProviderAll"><label for="providerAll" style="margin-left: 15px">{{ t('ui_view_all') }}</label>
        </p>
    </div>
    <div class="col-md-12">
        ...
        <div class="row" id="providersSection">
            @foreach($providers as $provider)
                ...
                @endif col-md-3 providers-section" data-provider-id="{{$provider->id}}" style="cursor:pointer;">
                    ...
                    @if($currentAdmin->isAllowed([\App\Enums\BUserPermissions::ADD_EDIT_PROVIDERS]))
                        <div class="editComplianceProvider" data-provider-id="{{ $provider->id }}">Edit</div>
                    @endif
                </div>
            @endforeach
        </div>
                        ...
                        <div class="modal-content" id="complianceProvider" style="border:none;border-radius: 5px;padding: 25px;width: 700px">
                            <div class="modal-body">
                                <form name="providerForm" id="providerForm"
                                      action="{{ route('backoffice.compliance.provider.store') }}" method="post">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}" id="providerToken">
                                    <h3>{{ t('provider_compliance_new') }}</h3>
                                    ...
                                    <div class="row">
                                    <div class="col-md-5">
                                        <label for="api" class="activeLink">{{ t('compliance_provider') }}</label><br>
...
@section('scripts')
                ...
                <script type="text/javascript">
                    $(document).ready(function () {
                        ...
                        $('body').on('click', '.editComplianceProvider', function () {
                            ...
                            $('#accountApi').text('');
                            $.ajax({
                                url: 'get-compliance-provider/'+providerId,
                                success: function (data) {
                                    ...
                                    if (!$("#paymentProviderForm input[name=_method]").val()) {
                                        ...
                                        $('form[name="providerForm"]').attr('action', '{{ route('backoffice.compliance.provider.update') }}')
                                    }
                        ...
                        function generateProviderAccounts(providerKey, account = null) {
                            ...
                            $.ajax({
                                url: 'get-compliance-provider-accounts/' + providerKey,
                                type: 'get',
                                success(data) {
                                    if (data.accounts) {
